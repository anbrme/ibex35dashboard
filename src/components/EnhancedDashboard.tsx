      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-white/80 backdrop-blur-sm border-b border-gray-200/50 p-4">
          <div className="flex gap-3">
            {[
              ...
              { id: 'news', label: 'News', icon: BarChart3, description: 'RSS news feed' },
              { id: 'lobbying', label: 'Lobbying', icon: Building2, description: 'Lobbying meetings' },
            ].map((tab) => {
        ...
        <div className="flex-1 p-6 bg-gray-50">
          <div className="bg-white rounded-lg shadow-sm p-6 h-full">
            ...
            {activeView === 'news' && (
              <div>
                <h2 className="text-lg font-semibold mb-4">Latest News (RSS Feeds)</h2>
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {news.length > 0 ? news.map((item) => (
                    <div key={item.id} className="border rounded-lg p-4 bg-white shadow-sm">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-semibold text-blue-900 hover:text-blue-700">
                    ...
                    <div className="text-center py-8 text-gray-500">
                      <p>Loading news...</p>
                      <p className="text-sm mt-2">Fetching latest RSS feeds from Google News</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {activeView === 'lobbying' && (
              <div>
                <h2 className="text-lg font-semibold mb-4">Lobbying Meetings</h2>
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {lobbyingMeetings.length > 0 ? lobbyingMeetings.map((meeting) => (
                    <div key={meeting.id} className="border rounded-lg p-4 bg-white shadow-sm">
                      <h3 className="font-semibold text-gray-900 mb-2">{meeting.organizationName}</h3>
                      <p className="text-gray-600 text-sm mb-2">{meeting.purpose}</p>
                      <div className="flex justify-between items-center text-xs text-gray-500 mb-2">
                        <span>{meeting.meetingDate.toLocaleDateString('es-ES')}</span>
                    ...
                    <div className="text-center py-8 text-gray-500">
                      <p>No lobbying meetings found</p>
                      <p className="text-sm mt-2">Lobbying data will appear here when available</p>
                    </div>
                  )}
                </div>
              </div>
